...
class PokeapiLocalDataSourceImpl implements PokeapiLocalDataSource {
  ...
  Future<List<PokemonModel>> getPokemonList({
    ...
    final queryResult = await database.rawQuery(pokemonListQuery, [
      searchQuery,
      '%$searchQuery%',
      limit,
      offset,
  ...
  Future<PokemonModel> getPokemon({
    ...
    final queryResult = await database.rawQuery(pokemonQuery, [id]);
    final result = queryResult.map(
      (row) => PokemonModel(
        id: row['id']! as int,
        name: row['name']! as String,
  ...
  Future<PokemonSpeciesModel> getPokemonSpecies({required int id}) async {
    final queryResult = await database.rawQuery(pokemonSpeciesQuery, [id]);
    final result = queryResult.map(
  ...
  Future<List<PokemonStatModel>> getPokemonStats({required int id}) async {
    final queryResult = await database.rawQuery(pokemonStatsQuery, [id]);
    final result = queryResult.map(
  ...
  Future<List<PokemonEggGroupModel>> getPokemonEggGroups({
    ...
    final queryResult = await database.rawQuery(pokemonEggGroupsQuery, [id]);
    final result = queryResult.map(
  ...
  Future<List<PokemonEvolutionModel>> getPokemonEvolutions({
    ...
    final queryResult = await database.rawQuery(pokemonEvolutionsQuery, [id]);
    final result = queryResult.map(
